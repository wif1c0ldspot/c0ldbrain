---
confidence: medium
created: 2026-04-18
priority: important
status: current
summary: Hierarchical memory system for AI agents — palace graph architecture with
  wings/rooms/drawers
tags:
- knowledge-management
- memory-systems
- mcp
title: MemPalace
type: concept
updated: '2026-04-26'
---

# MemPalace

Hierarchical memory system for AI agents — palace graph architecture with wings/rooms/drawers.

## Key Aspects

- Core principles and foundational concepts of MemPalace
- Practical implementation patterns and best practices
- Integration with related tools and frameworks
- Current research directions and emerging trends
- Trade-offs and design considerations for production use

## Related Concepts

- [[memory-systems]]
- [[context-substrate]]
- [[agent-memory-two-camps-witcheer-2026-04]]
- [[belief-gate]]
- [[claim-based-memory]]
- [[cognee]]
- [[honcho]]
- [[openchronicle]]

- [[openchronicle-einsia-2026-04]]
- [[openviking-context-database]]
- [[weekly-synthesis-async-subagent-2026-04-17]]